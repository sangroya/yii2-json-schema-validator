...
class ValidatorHelper implements IValidatorHelper
{
     ...
     * @inheritDoc
     */
    public function type($value, bool $use_integer = false,$expectedType=null): string
    {
        if (is_null($value)) {
            return "null";
        }
        ...
        if (is_string($value) && $expectedType=="date" && preg_match('/^' . '(?<fullyear>\d{4})-(?<month>0[1-9]|1[0-2])-(?<mday>0[1-9]|[12][0-9]|3[01])' . '$/i', $value, $m)) {
          
            if(checkdate($m['month'], $m['mday'], $m['fullyear']))
            return "date";
        }

        if (is_string($value) && $expectedType=="snumber") {
            return "snumber";
        }
        if (is_string($value)) {
            return "string";
    ...
    public function isValidType($value, $type): bool
    {
        ...
        $value_type = $this->type($value, true,$type);
        if (is_string($type)) {
